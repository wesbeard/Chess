...
public class King extends Piece {
    ...
    public boolean move(int targetX, int targetY, ArrayList<Piece> pieces, Piece toTake) {
        ...
        if (side == "dark" && !moved) {
            if((targetX == 1 && targetY == 0) && !blockedHorizontal(1, 0, pieces, toTake)) {
                castled = castle(pieces,1, 0, 0, 0, 2, 0);
            }
            else if((targetX == 6 && targetY == 0) && !blockedHorizontal(1, 0, pieces, toTake)) {
                castled = castle(pieces, 6, 0, 7, 0, 5, 0);
            }
        }
        // If light and not moved then check which side is being castled and pass ending coordinates
            ...
            if((targetX == 1 && targetY == 7) && !blockedHorizontal(1, 0, pieces, toTake)) {
                castled = castle(pieces, 1, 7, 0, 7, 2, 7);
            }
            else if((targetX == 6 && targetY == 7) && !blockedHorizontal(1, 0, pieces, toTake)) {
                castled = castle(pieces, 6, 7, 7, 7, 5, 7);
            }
        }
    ...
    public boolean castle(ArrayList<Piece> pieces, int castleX, int castleY, int rookX, int rookY, int rookX2, int rookY2) {
